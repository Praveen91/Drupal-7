<?php
$manis_skin = theme_get_setting('manis_skin', 'manis');
if(!empty($manis_skin)){
  $skin_color = '/css/skins/'.$manis_skin.'.css';
}else{
...
function manis_preprocess_html(&$variables) {
  //-- Google web fonts -->
...
function manis_preprocess_node(&$vars) {
  unset($vars['content']['links']['statistics']['#links']['statistics_counter']['title']);
}
function manis_form_alter(&$form, &$form_state, $form_id) {
...
function manis_breadcrumb($variables) {
  $crumbs ='';
...
function manis_preprocess_file_entity(&$variables) {
...
function manis_menu_tree__main_menu(array $variables) {
  return '<ul class="main-nav">' . $variables['tree'] . '</ul>';
}
function manis_menu_tree__menu_footer_menu($variables) {
  $str  = '';
...
function manis_menu_tree__menu_menu_404_page($variables) {
  $str  = '';
